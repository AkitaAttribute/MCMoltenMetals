...
- Raw-material, scrap, and ore conventions are supporting inputs only. They may provide artwork for an already-discovered matching metal, but they do not create separate molten materials by themselves. For example, bauxite does not become `Molten Bauxite` merely because it is a raw/ore material.
- The registered name/identity remains ingot-led, while palette artwork is chosen from the least-processed matching metal form we can identify: matching `raw_<metal>` artwork first, then matching `<metal>_scrap` artwork, then ingot artwork, then ore artwork. This makes Netherite use `netherite_scrap` for its palette without hard-coding Netherite itself.
...
- Vanilla lava is an artwork/animation template, not the identity of the registered fluid. The current lava still/flow animation is palette-remapped from the selected raw/scrap/ingot/ore source artwork.
