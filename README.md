...
- Known non-metal items that are incorrectly exposed through metal-style common tags can be excluded from discovery. `pixelmon:crystal` is currently ignored entirely, so it does not create a molten fluid or bucket.
